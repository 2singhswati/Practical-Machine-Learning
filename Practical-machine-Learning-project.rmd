...
Dataset: HAR [http://groupware.les.inf.puc-rio.br/har](http://groupware.les.inf.puc-rio.br/har) train a predictive model to predict what exercise was performed using a dataset.
...
There is a lot of data with NA / empty values. Let's remove those
...
let's convert all factors to integers
```{r}
...
Let's train some models to get closer to a way of predicting these classes.

## Model selection 
identify variables with high correlations amongst each other in the set, to possibly exclude them from the pca or training. 
...
Let's use 200 trees, as the error rate doesn't decline a lot after say 50 trees, but we still want to be thorough.
let's time each of the 4 random forest models to see if when all else is equal one pops out as the faster one.
